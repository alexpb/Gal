/* http://keith-wood.name/svg.html
   ...
   Update: Added support for jQuery 1.6
   Added quadratic Belzier curve path annimation
   */
...
(function ($) { // Hide scope, no $ conflict
	    ...
	    function (i, attrName) {
	        ...
	        $.fx.step['svg' + ccName] = $.fx.step['svg-' + attrName] = function (fx) {
	            ...
	            if (!fx.set) {
	                ...
	                if (fx.options.curAnim === undefined && fx.options.animatedProperties !== undefined) { fx.options.curAnim = fx.options.animatedProperties; }
    ...
    // Enable animation for path 
    $.fx.step['svgPath'] = $.fx.step['svg-path'] = function (fx) {
        var attr = fx.elem.attributes.getNamedItem('d');
        if (!fx.set) {
            fx.start = parseQuadraticBelzierCurvePath(attr ? attr.nodeValue : '');
            if (fx.options.curAnim === undefined && fx.options.animatedProperties !== undefined) { fx.options.curAnim = fx.options.animatedProperties; }
            var offset = fx.options.curAnim['svg-path'] || fx.options.curAnim['svgPath'];
            if (offset == 'swing') { offset = fx.end; }
            fx.end = parseQuadraticBelzierCurvePath(offset);
            fx.set = true;
        }
        ...
        (attr ? attr.nodeValue = value : fx.elem.setAttribute('d', value));

        //console.log('d: ' + d);
    };

    /* Parse a Path definition: Mx1,y1 Qcx,cy x2,y2
    @param  value  (string) the definition
    @return  (number[6]) the extracted values */
    function parseQuadraticBelzierCurvePath(data) {
        var parts = data.split(/[\s,]+/g);
        for (var i = 0; i < parts.length; i++) {
            //if the value is not a string means that is already parsed (because of the splice below)
            if (!parts[i].substring) { continue; }

            //check for letters
            var letter = parts[i].substring(0, 1);
            var num;
            ...
            if (!isNaN(parseFloat(letter))) {
                ...
                num = parseFloat(parts[i].substring(1));
            }

            if (!isNaN(num)) { parts[i] = num; }
            if (letter != '') { parts.splice(i, 0, letter); }
        }
